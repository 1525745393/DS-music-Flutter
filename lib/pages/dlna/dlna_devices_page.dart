...
class _DlnaDevicesPageState extends ConsumerState<DlnaDevicesPage> {
            ...
            Expanded(
              child: devices.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        final d = devices[i];
                        final isSelected = _controller.selectedRenderer?.uuid == d.uuid;
                        return _deviceItem(d, isSelected, current, state.position);
                      },
                    ),
            ),
          ],
  ...
  Widget _deviceItem(dynamic device, bool selected, Song? current, Duration position) {
    ...
    return GestureDetector(
      ...
      onTap: () async {
        ...
        if (current != null) {
          // 推送当前歌曲到设备
          // final streamUrl = ...
          // await _controller.push(current, streamUrl);
        }
        setState(() {});
      },
      child: Container(
